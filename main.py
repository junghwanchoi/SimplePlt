...
CFG_Common_drop_duplicates = False
...
class dlgChartType(QDialog):
    ...
    def __init__(self):
        ...
        # set focus
        self.tabWidget.setCurrentWidget(self.tabWidget.findChild(QWidget, "tab_plot"))
...
class MatplotlibWidget(QMainWindow):
    ...
    def OnBtnClick_Plt(self):
        ...
        if( len(SPL_dfData) > 0): # DataFrame을 읽어 들였다면
            ...
            if( (len(lstY1Attr) > 0) or (len(lstY2Attr) > 0) ): # Y1 에 그릴 데이터가 있다면
                ...
                # ------------------------------------------------------
                if (len(XAttr) > 0):  # X에 그릴 데이터가 있다면
                    x_data = dfData[XAttr].to_numpy()
                else: # if (len(XAttr) > 0):  # X에 그릴 데이터가 있다면
                    if (len(lstY1Attr) > 0):  # Y1 에 그릴 데이터가 있다면
                        x_data = np.arange(len(dfData[lstY1Attr[0]]))  # X 를 선택안했을 경우를 대비해서 X 축 데이터 만듦
                    elif (len(lstY2Attr) > 0):  # Y2 에 그릴 데이터가 있다면
                        x_data = np.arange(len(dfData[lstY2Attr[0]]))  # X 를 선택안했을 경우를 대비해서 X 축 데이터 만듦
                ...
                for lstYAttr_Index, lstYAttr in enumerate( [lstY1Attr, lstY2Attr] ):
                    ...
                    if (len(lstYAttr) > 0):  # Y1 에 그릴 데이터가 있다면
                                    ...
                                    if (len(lstLegendYValue_unique) > CFG_Legend_MaxMax):
                                        ...
                                        if YAttr in lstSelectedItemText:  # 현재 선택되어 있다면
                                            line, = matplotlib_axes.plot(
                                                ...
                                                y_data,
                                                '-o',
                                                linewidth=CFG_Plot_linewidth,
                                                markersize=CFG_Plot_markersize,
                                        ...
                                        for LegendYValue in lstLegendYValue_unique:  # legend 별 1개 씩 처리함
                                            if (LegendYValue not in lstLegend):  # 이전에 표시하지 않은 legend의 데이터라면.
                                                ...
                                                if YAttr in lstSelectedItemText:  # 현재 선택되어 있다면
                                                    line, = matplotlib_axes.plot(
                                                        [x_data[i] for i in npLegendDrawDataPos],
                                                        [y_data[i] for i in npLegendDrawDataPos],
                                                        '-o',
                                                        linewidth=CFG_Plot_linewidth,
                                                        markersize=CFG_Plot_markersize,
                                                        drawstyle=CFG_Plot_DrawType,
                                ...
                                if (YAttr in lstDrawY_Legend):
                                    ...
                                    if YAttr in lstSelectedItemText:  # 현재 선택되어 있다면
                                        line, = matplotlib_axes.plot(
                                            ...
                                            drawstyle=CFG_Plot_DrawType,
                                            alpha=CFG_Plot_Alpha_Selected,
                                            color=colorsY[i%len(colorsY)], # color=colorsY[lstSelectedItemText.index(YAttr)%len(colorsY)],
                                            picker=True, pickradius=5)  # 5 points tolerance
                                        line.set_label(YAttr)  # 'Figure Options' 창에서 Curve이름 표시하기 위해
                                        dicGlobalYLineToLegend[line] = YAttr
                                        lstGlobalYLine.append(line)
                        ...
                        if (len(lstLegAttr) > 0):  # 'L' 에 체크가 되었다면
                            ...
                            for i, YAttr in enumerate(lstYAttr):
                                if YAttr not in lstDrawY_Legend:
                                    ...
                                    if (len(lstLegendYValue_unique) > CFG_Legend_MaxMax):
                                        ...
                                        for j, Legend in enumerate(lstLegendYValue_unique):
                                            ...
                                            if YAttr in lstSelectedItemText:  # 현재 선택되어 있다면
                                                line, = matplotlib_axes.plot(
                                                    [x_data[i] for i in lstDataPos],
                                                    [y_data[i] for i in lstDataPos],
                                                    '-o',
                                                    linewidth=CFG_Plot_linewidth,
                                                    markersize=CFG_Plot_markersize,
                                                    drawstyle=CFG_Plot_DrawType,
                ...
                if (len(lstY2Attr)>0) or (len(lstY1Attr)>0): # 그릴 데이터가 있을때
                    if (XAttr == "column_index"):
                        ...
                        if (len(lstColIndex) > 200):
                            self.MplWidget.canvas.axes_1.set_xticks(np.arange(min(lstColIndex), max(lstColIndex)+1, 5))
                            self.MplWidget.canvas.axes_2.set_xticks(np.arange(min(lstColIndex), max(lstColIndex)+1, 5))
                        elif (len(lstColIndex) > 100):
                            self.MplWidget.canvas.axes_1.set_xticks(np.arange(min(lstColIndex), max(lstColIndex)+1, 3))
                            self.MplWidget.canvas.axes_2.set_xticks(np.arange(min(lstColIndex), max(lstColIndex)+1, 3))
                        elif (len(lstColIndex) > 30):
                            self.MplWidget.canvas.axes_1.set_xticks(np.arange(min(lstColIndex), max(lstColIndex)+1, 2))
                            self.MplWidget.canvas.axes_2.set_xticks(np.arange(min(lstColIndex), max(lstColIndex)+1, 2))
                        else:
                            self.MplWidget.canvas.axes_1.set_xticks(np.arange(min(lstColIndex), max(lstColIndex)+1, 1))
                            self.MplWidget.canvas.axes_2.set_xticks(np.arange(min(lstColIndex), max(lstColIndex)+1, 1))
    ...
    def onMplMouseDown(self, event):
        ...
        if((self.last_artist != None) and (self.last_mouse == "Up")) or (SPL_boForceDraw==True) :
            ...
            for line in self.dicY1LineToLegend.keys():
                line.set_alpha( CFG_Plot_Alpha_Unselected ) # 원래 투명도
            self.last_artist = None # 마지막 클릭한 선 정보 지움
    ...
    def onMplPick(self, event):
        ...
        if( CFG_Common_PickMode==COMMON_PICKMODE_LEGEND ):
            ...
            # annotion 표시
            Annotation_one = self.MplWidget.canvas.axes_1.annotate(
                                self.dicY1LineToLegend[thisline],
                                xy=(event.mouseevent.x, event.mouseevent.y - 20*self.number_annotation_one_click),
                                xycoords='figure pixels')
            self.lstAnnotation.append(Annotation_one)
            self.number_annotation_one_click += 1
